...
with open('/Users/liuhui/Desktop/COVID19/data/pycharts_city.txt', 'r', encoding='UTF-8') as f:
...
m.set_series_opts(label_opts=opts.LabelOpts(font_size=12),
                  is_show=False)
m.set_global_opts(title_opts=opts.TitleOpts(title='湖北省实时确诊数据' + today,
                                            subtitle='数据来源：丁香园'),
                  legend_opts=opts.LegendOpts(is_show=False),
                  visualmap_opts=opts.VisualMapOpts(pieces=pieces,
                                                    is_piecewise=True,
                                                    is_show=True))
m.render(path='/Users/liuhui/Desktop/COVID19/html/湖北省实时确诊数据地图' + today + '.html')